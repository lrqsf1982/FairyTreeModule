...
public:
	...
	//设置小精灵状态
	void	SetPlunderState(ElfinState state);

	//获得小精灵状态
	ElfinState GetPlunderState()const;

	//设置被掠夺者ID
	void SetBePlunderId(uint32);

	//获取被掠夺者ID
	uint32 GetBePlunderId()const;

	//获取已经掠夺的金币数量
	uint32 GetPlunderGoldNum()const;
private:
